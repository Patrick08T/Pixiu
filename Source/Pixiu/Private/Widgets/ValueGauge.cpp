...
void UValueGauge::SetAndBoundToGameplayAttribute(class UAbilitySystemComponent* ASC, const FGameplayAttribute& Attribute, const FGameplayAttribute& MaxAttribute)
{
	if (ASC)
	{
		bool bFound = false;
		float CurrentValue = ASC->GetGameplayAttributeValue(Attribute, bFound);
		float CurrentMaxValue = ASC->GetGameplayAttributeValue(MaxAttribute, bFound);
		if (bFound)
		{
			SetValue(CurrentValue, CurrentMaxValue);
		}
